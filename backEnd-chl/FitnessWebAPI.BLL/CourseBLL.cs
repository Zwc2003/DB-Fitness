...
{
    //public string PublishCourse() //涉及的表有： Course , Teaches
    //PublishCourse
    //ModifyCourse
    ...
    //ReserveCourse  //涉及的表有：Book  
    //PayCourseFare    //涉及的表有： Payment Trainee Participate //如果user报名课程，则向Traniee表中插入一项   
    //CancleCourse   //涉及的表有：Book Participate
    //GetCourseByUserID

    //SearchCourse:根据查询参数进行搜索
    //GradeCourse //涉及的表有：Participate
    //PublishComment  //涉及的表有：Participate
    
    //GetCourseCommentByClassID
    ...
    public class CourseBLL : ICourseBLL
    {
        ...
        public string DeleteCourseByClassID(string token,int classID)
        {
            ...
            /*      // 删除教练授课信息:  设置级联删除后会自动删除
                    if (!TeachesDAL.Delete(tokenRes.userID, classID, transaction))
                    {
                        transaction.Rollback();
                        return "课程删除失败：无法删除教练授课信息";
                    }*/
...
/*            if (classID <= 0)
        ...
        //第一次预约课程：弹窗询问，确认后将加入Traniee表，之后将不再询问
        ...
        public string ReserveCourse(string token ,int classID, string payMethod)
        {
            try
            {
                TokenValidationResult tokenRes = _jwtHelper.ValidateToken(token);
                // 预订课程
                bool result = BookDAL.Insert(new Book
                {
                    classID = classID,
                ...
                {
                    return $"课程预订失败: {result}";
                }
            }
            catch (Exception ex)
            {
                return $"课程预订失败: {ex.Message}";
            }
        }
...
/*            finally
        ...
        //获取评分
        /*public int GetGrade(string token,int )*/
